...
impl SseParser {
    ...
    fn line(&mut self, raw: &[u8], events: &mut Vec<SseEvent>) -> Result<(), ModelError> {
        ...
        let line = std::str::from_utf8(raw).map_err(|_| {
            ModelError::invalid_response("Open Responses SSE contains invalid UTF-8")
...
async fn read_live(live: &mut Live) -> Result<(), ModelError> {
    if live.events.is_empty() {
        let next = tokio::select! {value=tokio::time::timeout(live.idle,live.bytes.next())=>value.map_err(|_|ModelError::timeout("Open Responses stream idle timeout").with_stage(ErrorStage::StreamRead).with_bytes_received(live.count))?,_=live.abort.aborted()=>return Err(ModelError::abort("Open Responses stream was aborted").with_stage(ErrorStage::StreamRead).with_bytes_received(live.count))};
...
async fn read_error_body(
    ...
    let mut count = 0_u64;
    loop {
        let next = tokio::select! {value=tokio::time::timeout(idle,stream.next())=>value.map_err(|_|ModelError::timeout("Open Responses error body idle timeout").with_stage(ErrorStage::ResponseBody).with_bytes_received(count))?,_=abort.aborted()=>return Err(ModelError::abort("Open Responses error body read was aborted").with_stage(ErrorStage::ResponseBody).with_bytes_received(count))};
        match next {
            Some(Ok(chunk)) => {
            ...
            Some(Err(_)) => {
                return Err(
                    ModelError::transport("Open Responses error body read failed")
