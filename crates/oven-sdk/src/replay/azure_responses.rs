...
const REPLAY_FORMAT: &str = "oven.azure.openai.responses.output.v4";

/// Normalized integrity witness used by Azure Responses capture.
pub const FINGERPRINT_KIND: &str = "azure.openai.responses.replay_fingerprint";

/// Validates and fingerprints Azure Responses output items for native capture.
pub fn capture(items: &[JsonValue]) -> Option<(Vec<JsonValue>, String)> {
    let sanitized = items
        .iter()
        .map(sanitize_item)
...
pub(crate) fn decode(
    ...
    let witnesses = normalized
        .iter()
        .filter_map(|part| match part {
            AssistantPart::Custom(part) if part.kind == super::AZURE_RESPONSES_CONTINUATION => {
    ...
    if !witnesses.is_empty()
        && witnesses
            != items
                .iter()
                .filter_map(super::azure_responses_continuation)
                .map(|part| part.data)
                .collect::<Vec<_>>()
    {
        return None;
